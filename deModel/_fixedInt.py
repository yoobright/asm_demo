...
class DeFixedInt(object):
    ...
    def __mul__(self, other):
        ...
        if (isinstance(other, DeFixedInt)):
            ...
            b = DeFixedInt(self.__intWidth, self.__fractWidth, other,
                           self.__roundMode)
            retValue = self * b

        else:
            msg = "'%s' not supported as operator for DeFixedInt multiplication" % type(
                other)
            raise TypeError, msg
    ...
    def __lshift__(self, other):
        ...
        if (not isinstance(other, (int, long))):
            msg = "unsupported operand type(s) for <<: 'DeFixedInt' and '%s'" % type(
                other)
            raise TypeError, msg
    ...
    def __rshift__(self, other):
        ...
        if (other > 0):
            if (self.__roundMode == 'round'):
                ...
                if (
                    msbTrunc and not remainTrunc.value):  # truncing 100..-> round even
                    retValue.value = (self.value >> other) + \
                                     (newBitZero & msbTrunc)
    ...
    def bit(self):
        ...
        width = self.width
        mask = long(2L ** width) - 1
        # fStr = '0x%%.%dX' % (int(width))
        fStr = '{0:0%db}' % (int(width))
        return fStr.format(self.value & mask)
